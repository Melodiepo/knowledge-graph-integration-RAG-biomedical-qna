...
def query_preprocess_instruction(input_path, use_spacy = True): #using spacy to divide query into sentences and then inserting [SEP] token is time-intensive, so we leave it as user's design choice.
...
def query_encode(input_list):
    ...
    splits = [i for i in range(0, len(input_list), 1)]
    for i in tqdm(splits, desc="query encoding"):
        split_queries = input_list[i:i+1]
        with torch.no_grad():
            encoded = tokenizer(
                split_queries, 
                truncation=True, 
