...
def statpearls_index_create(statpearls_embeddings_dir):
    ...
    with tqdm(total=1, desc="statpearls load and add", dynamic_ncols=True) as pbar:
        embeddings = np.load(os.path.join(statpearls_embeddings_dir, "Statpearls_Total_Embeds.npy")) 
        embeddings = embeddings.astype(np.float32)
...
def pubmed_decode(pubmed_I_array, pubmed_articles_dir, pubmed_group_num):
    ...
    #pubmed_I_array = np.load(idx_array_savepath)
        ...
        for ith, indices in tqdm(enumerate(pubmed_I_array[start_index//pubmed_group_num]), desc="decode and add", dynamic_ncols=True):
...
def statpearls_decode(statpearls_index, statpearls_articles_dir):
    def load_articles(statpearls_articles_dir):
        with open(os.path.join(statpearls_articles_dir, "Statpearls_Total_Articles.json"), 'r') as jsfile:
            statpearls_articles = json.load(jsfile)
        return statpearls_articles
    ...
    for ith, indices in tqdm(enumerate(statpearls_index), desc="decode and add", dynamic_ncols=True):
        evidence_list = [find_value_by_index(statpearls_articles, j) for j in indices]
