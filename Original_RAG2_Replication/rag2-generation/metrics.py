...
def evaluate_medqa(model_json, ground_truth_json):
    ...
    ground_truth_dict = {idx: item["answer_idx"] for idx, item in enumerate(ground_truth_data)} # Map question_idx to answer_idx
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate MedQA Model Accuracy")
    parser.add_argument("--model_json", required=True, help="Path to model output JSON")
    parser.add_argument("--ground_truth_json", required=True, help="Path to ground truth JSON")
    
    args = parser.parse_args()
    evaluate_medqa(args.model_json, args.ground_truth_json)