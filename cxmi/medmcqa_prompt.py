...
def main(filtered_docs_dir, answers_dir, output_path):
    ...
    for idx, content in tqdm(enumerate(answers_data), total=len(answers_data), desc="Prompt Generating"):
        ...
        options_dict = {
            "A": answers_data[idx].get("opa", ""),
            "B": answers_data[idx].get("opb", ""),
            "C": answers_data[idx].get("opc", ""),
            "D": answers_data[idx].get("opd", ""),
        }
        if options_dict["A"] == "":
...
if __name__ == "__main__":
    ANSWER_PATH = "/cs/student/projects1/ml/2024/yihanli/retriever/input/medmcqa/dev.json"

    sent_list = ["passage", "sent"]
    flant5_list = ["base", "large"]
    for sent_pas in sent_list:
        for flant5 in flant5_list:
            FILTERED_DOC_PATH = f"/cs/student/projects1/ml/2024/yihanli/filco/output/medmcqa_cot/cxmi_{sent_pas}_pruning_{flant5}_k_10.json"
            OUTPUT_PATH = f"/cs/student/projects1/ml/2024/yihanli/filco/output/medmcqa_cot/cxmi_{sent_pas}_prompt_{flant5}_k_10.json"
            main(FILTERED_DOC_PATH, ANSWER_PATH, OUTPUT_PATH)
            gc.collect()

    FILTERED_DOC_PATH = "/cs/student/projects1/ml/2024/yihanli/similarity_pruning/output/medmcqa_cot/similarity_pruning_test_k_10.json"
    OUTPUT_PATH = "/cs/student/projects1/ml/2024/yihanli/similarity_pruning/output/medmcqa_cot/similarity_prompt_test_k_10.json"
    main(FILTERED_DOC_PATH, ANSWER_PATH, OUTPUT_PATH)
